...
// Test Marker and MyData recorder event messages
bool TestDisplay::onMarkerKey()
{
   BEGIN_RECORD_DATA_SAMPLE( getStation()->getDataRecorder(), REID_MARKER )
      SAMPLE_3_VALUES( 1, 1, fooCnt++ )
   END_RECORD_DATA_SAMPLE()

   unsigned int fee = myDataCnt*myDataCnt;
   myDataCnt++;

   unsigned int fi = myDataCnt*myDataCnt;
   myDataCnt++;

   unsigned int fo = myDataCnt*myDataCnt;
   myDataCnt++;

   BEGIN_RECORD_DATA_SAMPLE( getStation()->getDataRecorder(), REID_MY_DATA_EVENT )
      SAMPLE_3_VALUES( fee, fi, fo )
   END_RECORD_DATA_SAMPLE()
   return true;
}
    ...
    // Now add any new tracks
    {
        ...
        while (inew < nNewTracks && islot < maxTracks) {
            if (codes[islot] == 0) {
                ...
                else if (newTracks[inew]->isClassType(typeid(models::IMissile))) {
                   type = 5; // Missile
                }
...
void TestDisplay::updatePfd(const double)
{
    ...
    double vvMps = -vel[2];
    vvi = vvMps * 60.0f * base::length::M2FT;
